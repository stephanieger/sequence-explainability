...
'''
This assumes getExplainabilityGradBatches.py or getExplainabilityBatchedGradBatches.py has been run, get 
explanation vector for all data as well as positive reviews. 
'''
...
parser.add_argument("grad_folder", help="type of grad, grad/ or int-grad/", type=str)
...
if args.grad_folder == 'grad/':
	atten_base = args.model_folder + args.grad_folder + 'X_' + args.val + '_grad_batch_'
elif args.grad_folder == 'int-grad/':
	atten_base = args.model_folder + args.grad_folder + args.val + '_grad_batch_'
...
# save arrays
if args.grad_folder == 'grad/':
	np.save(save_folder + args.val + '_churn_grad_vec.npy', churn_atten_vec)
	np.save(save_folder + args.val + '_grad_vec.npy', atten_vec)
	names = ['churn_grad', 'grad']

elif args.grad_folder == 'int-grad/':
	np.save(save_folder + args.val + '_churn_int_grad_vec.npy', churn_atten_vec)
	np.save(save_folder + args.val + '_int_grad_vec.npy', atten_vec)
