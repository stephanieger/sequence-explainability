...
def optLoss(w_target, w_hat, _dir, grad=None):
	...
	_div = tf.divide(num, den + tf.keras.backend.epsilon()) #-num / den
	loss = tf.negative(_div)#tf.negative(tf.pow(_div, opt_pow*tf.ones_like(_div)))
	return loss
...
def opt(w_target, w_hat, opt_model, con_eps, _lambda=_lambda, learning_rate = learning_rate,
	...
	grad = getGrad(w_hat, _idx)  # idx[grad_batch])
	loss_old = 10000 #np.squeeze(K.eval(optLoss(w_target, w_hat, weights, grad=grad, _lambda=_lambda, _s=_s)))  #getLoss(w_hat)
	...
	print('loss:', loss_new)#abs(loss_old - loss_new))
	grad_list = list()
