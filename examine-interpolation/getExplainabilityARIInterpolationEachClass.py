...
'''
Compute ARI score along valley tracking low-loss path individually for each class.
'''
...
parser.add_argument("--n_clusters", help='number of clusters', type=int, default=2)
...
for i, atten_dir in enumerate(atten_dirs):
	...
	for j in range(num_steps-1):
		...
		dat_i = np.load(
			atten_dir + 'interp-step-' + str(j) + '/explainability/' + args.val + '_churn_atten_vec.npy')
		dat_j = np.load(
			atten_dir + 'interp-step-' + str(j + 1) + '/explainability/' + args.val + '_churn_atten_vec.npy')
...
for atten_dir in atten_dirs:
	if args.feat:
		np.save(atten_dir + 'churn_feat_ARI.npy', ARI)
	else:
		np.save(atten_dir + 'churn_ARI.npy', ARI)
...
for i, atten_dir in enumerate(atten_dirs):
	...
	print(num_steps)
	for j in range(num_steps-1):
		dat_i = np.load(atten_dir + 'interp-step-' + str(j) + '/explainability/' + args.val +
						'_non_churn_atten_vec.npy')
		dat_j = np.load(atten_dir + 'interp-step-' + str(j + 1) + '/explainability/' + args.val +
						'_non_churn_atten_vec.npy')
...
for atten_dir in atten_dirs:
	np.save(atten_dir + 'non_churn_ARI.npy', ARI)
