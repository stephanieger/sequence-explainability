...
cluster_data_dir = save_folder + 'no-churn-cluster-dat/'
if not os.path.exists(cluster_data_dir):
	os.makedirs(cluster_data_dir)
cluster_dir = [save_folder + 'no-churn-cluster-' + str(t) + '/' for t in range(2)]
...
num_steps = len(glob.glob(args.cluster_lab_folder + 'non_churn_cluster_labels_*.npy'))
init_label = np.load(args.cluster_lab_folder + 'non_churn_cluster_labels_0.npy')
labels = []
labels += [init_label]

for num in range(0, num_steps):
	lab = np.load(args.cluster_lab_folder + 'non_churn_cluster_labels_' + str(num) + '.npy')
...
dat = np.load(atten_dirs[0] + 'interp-step-0/explainability/' + args.val + '_non_churn_atten_vec.npy')
DAT = [dat[idx_zero], dat[idx_one]]

np.save(cluster_data_dir + 'NON_CHURN_DAT_' + str(cnt) + '.npy', DAT)
...
np.save(save_folder + 'NON_CHURN_KEYS.npy', KEYS)
...
for i, atten_dir in enumerate(atten_dirs):
	...
	for j in range(num_steps):
		...
		dat = np.load(atten_dir + 'interp-step-' + str(j) + '/explainability/' + args.val + '_non_churn_atten_vec.npy')
		DAT = [dat[idx_zero], dat[idx_one]]
		MEAN_VEC = [np.mean(t, axis=0) for t in DAT]
