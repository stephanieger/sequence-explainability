...
'''
Compute ARI with respect to endpoint for each set of weights in valley tracking low-loss path. 
'''
...
dat_i = np.load(atten_dirs[0] + 'interp-step-0/explainability/' + args.val + '_atten_vec.npy')
...
for atten_dir in atten_dirs:
	np.save(atten_dir + 'ARI-from-endpoint.npy', ARI)