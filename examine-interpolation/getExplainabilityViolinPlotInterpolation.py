...
cluster_data_dir = save_folder + 'cluster-dat/'
if not os.path.exists(cluster_data_dir):
	os.makedirs(cluster_data_dir)
cluster_dir = [save_folder + 'cluster-' + str(t) + '/' for t in range(2)]
...
num_steps = len(glob.glob(args.cluster_lab_folder + 'cluster_labels_*.npy'))
init_label = np.load(args.cluster_lab_folder + 'cluster_labels_0.npy')
labels = []
labels += [init_label]

for num in range(0, num_steps):
	lab = np.load(args.cluster_lab_folder + 'cluster_labels_' + str(num) + '.npy')
...
dat = np.load(atten_dirs[0] + 'interp-step-0/explainability/' + args.val + '_atten_vec.npy')
DAT = [dat[idx_zero], dat[idx_one]]

np.save(cluster_data_dir + 'DAT_' + str(cnt) + '.npy', DAT)
...
np.save(save_folder + 'KEYS.npy', KEYS)
...
for i, atten_dir in enumerate(atten_dirs):
	...
	for j in range(num_steps):
		...
		dat = np.load(atten_dir + 'interp-step-' + str(j) + '/explainability/' + args.val + '_atten_vec.npy')
		# print(dat.shape)
		DAT = [dat[idx_zero], dat[idx_one]]
		# print(DAT[0].shape)
		MEAN_VEC = [np.mean(t, axis=0) for t in DAT]
