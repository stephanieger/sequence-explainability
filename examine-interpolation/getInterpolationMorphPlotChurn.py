...
'''
Plot how cluster morphs along interpolation plot for positive reviews. Run after interpolation path is determined. 
'''
...
for i, atten_dir in enumerate(atten_dirs):
	...
	for j in range(num_steps - 1):
		...
		dat_i = np.load(atten_dir + 'interp-step-' + str(j) + '/explainability/' + args.val + '_churn_atten_vec.npy')
		dat_j = np.load(atten_dir + 'interp-step-' + str(j + 1) + '/explainability/' + args.val + '_churn_atten_vec.npy')
		cnt += 1
		...
		if i == 0:
			if j == 0:
				np.save(args.save_folder + 'churn_cluster_labels_0.npy', cluster_labels_i)
		np.save(args.save_folder + 'churn_cluster_labels_' + str(cnt) + '.npy', cluster_labels_j)
		np.save(args.save_folder + 'churn_col_ind_' + str(cnt) + '.npy', col_ind)
